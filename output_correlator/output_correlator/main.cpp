...
uint32_t loadInput(uint8_t * buffer, char * filename, uint32_t numBytesToRead){
  using namespace std;
  FILE * hnd = fopen(filename,"r");
  if (hnd != NULL){
    uint32_t bytesRead = std::fread(buffer,sizeof(uint8_t),numBytesToRead,hnd);
    fclose(hnd);
    return bytesRead;
  } else return 0;
}
__attribute__((optimize("unroll-loops")))
void cross_correlate(uint8_t * a, uint8_t * b, uint64_t * out, uint32_t size_a, uint32_t size_b){
...
__attribute__((optimize("unroll-loops")))
void parallel_cross_correlate(uint8_t * a, uint8_t * b, uint64_t * out, uint32_t size_a, uint32_t size_b){
  assert(size_a != 0 && size_b != 0);
...
int main(int argc, char **argv) {
    ...
    uint8_t * stream1 = (uint8_t *)malloc(numSamples * sizeof(uint8_t));
    uint8_t * stream2 = (uint8_t *)malloc(numSamples * sizeof(uint8_t));
    ...
    uint64_t * correlation = (uint64_t*)malloc(sizeof(uint64_t)*numSamples);
    timer::tic();
    ...
    */
    ...
    uint64_t * par_correlation = (uint64_t*)malloc(sizeof(uint64_t)*numSamples);
    timer::tic();
  ...
  /*  
    ...
    */
    ...
    uint64_t maxV = par_correlation[0];
    uint64_t maxA = 0;
