...
In order to asses the quality of reconstruction we used Gaussian noise as input to the analysis filter bank and measured the Signal to Noise Ratio \gls{snr}. Our results 
confirm our initial fears about the quality of reconstruction: the basic filter bank construction works well for analysis but does not achieve the desired results in synthesis.
That said, there is a reasonable correlation between the input and the output of the synthesis filter bank. This can be confirmed by visual inspection of the (time-shifted) 
