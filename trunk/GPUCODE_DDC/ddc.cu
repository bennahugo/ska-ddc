...
int main(int argc, char **argv) {
 ...
 if (argc > 3) {
  ...
  output_file = argv[3];
 } else { printf("Please supply both data, fir_taps and output filenames...\n"); return -1;}

 printf("Actual lo freq is: %f MHz (%i, %i, %i, %i)\n", lo_interval / ((float)sin_table_length / sample_rate), lo_interval, sin_table_length, sample_rate, lo_freq);
 printf("Producing %i output samples per block (%i samples).\n",no_output_samples,write_block);
 ...
 printf("Copying FIR taps and SIN lookup table to GPU...\n");
 ...
 printf("GPU Configuration: blocks per grid: %i, threads per block: %i\n",BLOCKS_PER_GRID, THREADS_PER_BLOCK);
 ...
 for (int i=0; i < LOOPS; i++) {
  ...
  for (int j=0; j < 10; j++) { printf("Expanded: Char buffer value %i is %i\n",j, host_char_buffer[j]); }
  
  //output DDC data
  FILE * outputF = fopen(output_file,"w");
  if (outputF != NULL){
  	if (fwrite(host_char_buffer,sizeof(char), no_output_samples, outputF) != no_output_samples)
	   fprintf(stderr,"Could not dump output to file %s\n",output_file);
	fclose(outputF);
  } else {
    fprintf(stderr,"Could not dump output to file %s\n",output_file);
  }
  //write(1, output_buffer, sizeof(float) * no_output_samples);
  printf("Loop done. CUDA time is %f ms\n", et);
 }
