...
#define PACKETS_TO_BUFFER 1    // seems to be the sweet spot for software raid
#define PAYLOAD_SIZE 4096       // hardcoded for now
#define MMAP_SIZE 5000
...
// SPEAD counters
  ...
  while (num_pkts < pkts_expected) {
	...
	if (num_pkts == 0) spead_id_zero = heap_cnt;
       ...
       if (output_filename != NULL) {
       if (num_pkts % MMAP_SIZE == 0) {
        if (num_pkts > 0) munmap(dst, PAYLOAD_SIZE * MMAP_SIZE);
        number_writes++;
	lseek(outfh, number_writes * PAYLOAD_SIZE * MMAP_SIZE - 1, SEEK_SET); //write_size * number_writes - 1, SEEK_SET);
        write(outfh,"",1);
        dst = (char *)mmap(NULL, PAYLOAD_SIZE * MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, outfh, PAYLOAD_SIZE * MMAP_SIZE * (number_writes-1)); //write_size, PROT_READ | PROT_WRITE, MAP_SHARED, outfh, write_size * (number_writes-1));        
        if (dst == MAP_FAILED) { perror("Memory map failed"); exit(1); }
        //memcpy(dst, spead_buffer, write_size);
        //munmap(dst, write_size);
       ...
       memcpy(dst + (num_pkts % MMAP_SIZE) * 4096, recv_req.pkt_addr+UDP_HDRLEN+spead_header_size, recv_req.length - UDP_HDRLEN - spead_header_size); 
       }
       else write(1, (unsigned char*)(recv_req.pkt_addr+UDP_HDRLEN+spead_header_size), recv_req.length - UDP_HDRLEN - spead_header_size);
      num_pkts++;
